...
async function PATCH(req: Request, context: Context) {
  const params = context.params || {};
  const source_id = params['source_id'] as string | undefined;
  if (!source_id) return json({ error: 'source_id is required' }, 400);
  ...
  const updated = await db.update(sources).set(updateValues).where(eq(sources.id, source_id)).returning();

  if (!updated.length) return json({ error: 'Source not found' }, 404);
  return json(updated[0], 200);
}

async function DELETE(_req: Request, context: Context) {
  const params = context.params || {};
  const source_id = params['source_id'] as string | undefined;
  if (!source_id) return json({ error: 'source_id is required' }, 400);
